...
def getprice_quote(request):
	...
	try:
		with connection.cursor() as cursor:
			...
			for j in cat_name:
				sql1="select ca.CategoryName,sum(v.`Quotation(Exc Taxes)`) as `Quotation(Exc Taxes)` from  Presentation p inner join slide_elements s on p.s_id=s.s_id and s.flag=0 inner join Catalog c  on c.idCatalog=s.e_id inner join Category ca on ca.idCategory=c.idCategory left join VendorPieceQuotation v on c.idCatalog=v.idCatalog where p.pr_id='%s' and ca.CategoryName='%s'"%(pr_id,j)
				cursor.execute(sql1)
				res2=cursor.fetchall()

				if res2:
					for r in res2:
						if(r['Quotation(Exc Taxes)']!=None):
							sum_value=int(r['Quotation(Exc Taxes)'])
							total_sum=(dict(cat_name=j,cost=sum_value))
							cost.append(total_sum.copy())
						else:
							sum_value=0

			total_cost=dict(price = res,sum_val = sum_val)
		connection.commit()
	except Exception as e:
		print(e)
	finally:
		connection.close()
	return dict(cat_cost=cost,total_cost=total_cost)